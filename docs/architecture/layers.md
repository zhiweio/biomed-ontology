...
| 问题 | 层 | 例子 |
|---|---|---|
| 赛沃替尼 = AZD6094 = savolitinib？ | L1 术语 + L3 身份 | 别名、层级、外部 xref |
...
| HMPL-504 对应哪个候选药？ | L3 IdentityService | `HMD:ENT:DC:savolitinib` |

只做实体归一化是**检索索引**；加上术语层级、类型化关系、事实、质量、许可、可还原引用与企业主键，才构成 **AI-Ready Scientific Data Foundation** 及其 Ontology Semantic Layer。这是设计决策 D12 的落地（见 [附录 · 决策索引](../appendix/decisions.md)）。

L1–L5 不是「检索管道的前几步」，而是语义层本体能力本身。L6 只是把这些能力暴露出去的 Semantic Access。

对外名称是 **AI-Ready Scientific Data Foundation**，不要自称「企业知识图谱项目」。
业界六层栈与本仓库落点：
...
| Scientific KG | GraphDB named graphs（只认 GraphDB） |
...
| `IdentityService` 单一句柄 | 词典只装配一次 | `open_dual_surface` 维护两套字典 |
...
L3 身份          IdentityService：文本 → CURIE/ENT；Foundation 叠加 BERN2 / Zingg / xref
...
flowchart TB
  ...
  L1 --> L3[L3 IdentityService]
  L0 --> L4[L4 Corpus + IngestQA + Facts]
  L3 --> L4
...
| L3 | `identity.py`、`normalize/`、`foundation/resolve.py` | 文本到 ENT | [IdentityService](../ontology/identity.md) |
| L4 | `parse/`、`corpus/`、`lake/` | PDF → 语义树 → IngestQA → 事实 | `lake/ingest.py`、`corpus/extract.py` |
...
│   └── IdentityService.from_world()          # 目录 Normalizer + EntityResolver
├── pipeline.build_literature_base()          # 默认 with_graph=False
│   ├── ingest.catalog.catalog_files() → ontology/catalog/
...
└── foundation.api.FoundationApi              # get_entity_context → Context Pack
```

生产装配只走 `build_literature_base()`。`id_mode=ledger` 仅单测夹具。
...
| 归一化 | `IdentityService.normalize`（目录级联） | `IdentityService.resolve_text`（同一目录 + BERN2 / Zingg / xref） |
...
| 查询 API | `ToolApi.search` 等 8 工具 | Context Pack（`get_entity_context`） |

两层共享 GraphDB 运行时、LinkML 契约与 **同一份 IdentityService**。查询路径不混用 YAML fallback（Foundation 强制三后端）。
...
uv run pytest tests/test_walk_neighbors.py tests/test_identity.py -q
uv run hmd demo --id D7
```

相关文档：[Foundation](foundation.md)、[Pipeline](pipeline.md)、[IdentityService](../ontology/identity.md)、[不变量](../invariants.md)。
