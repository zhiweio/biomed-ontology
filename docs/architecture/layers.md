...
L6 Tool API      单一 `hmd serve`：KB 工具（ToolApi）+ Foundation Semantic Ops
...
| L6 | `tools/`、`service/`、`foundation/api` | 单一 `hmd serve`：ToolApi + Semantic Ops | `tools/api.py`、`foundation/api.py` |
...
| Tool API 里手写一份别名表 | L6 + L1 | 只调 `Normalizer` / `expand` |
...
| actions / functions | ToolApi 8 工具 | 契约 + 不可绕过包裹链 |
