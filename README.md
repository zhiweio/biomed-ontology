...
（`bridge_zh` / `alias` / `hierarchy` / `control` / `image` / `license`），
**judged@10 = 1.000**。判定粒度是章节 → **Recall@10 上限 0.837**（不是 1.0）。
键用 `scripts/dump_sections.py` 对照；dangling 键拒绝整次评测。

**主 KPI：本体敏感探针**（`bridge_zh` + `alias`，n=9）—— 别名归一与中文→英文桥接：
...
本体敏感探针 nDCG@10 绝对增益 **+0.078**（T1 门槛 +0.05，已达成）。
配对检验仍不显著（n=9，CI 跨零）—— 点估计过线 ≠ 可写成定论；对外须带 CI。
...
图像意图那 12 条上，本体臂与无本体臂**几乎逐位相同** —— 概念挂不到图切片，
它们只是往总平均里灌接近零的差值，把区间往零压。
检索侧的改造要读，就读敏感探针 / 文本意图；视觉列的改造要读，就读图像意图。
...
**一条都不显著。** 敏感探针上的 +0.078 过了 T1 门槛，但仍不得写成"本体已被证明"；
须同时给出 CI。全量口径更是诊断项。主指标取 nDCG@10 而非 Recall@10，
因为后者在这份 gold 上有 0.837 的天花板。

规模是硬约束：14 篇 / 588 切片 / 84 概念上 BM25 已接近饱和。
**能改的是 KPI 是否对准机制，以及机制对不对，不是硬拧全量 +10%。**
...
而这正是敏感探针 KPI 要的那种可归因证据。把 +0.037 拆成两笔（nDCG@10，n=25）：
...
登记在 `licensing.COMPONENTS["biomedclip"]`，`review=pending`。
PoC 默认 `accept_uncleared_components=true`（启动告警留痕），`hmd eval` /
`hmd index` 可直接跑；生产务必 `HMD_ACCEPT_UNCLEARED_COMPONENTS=false`。
...
T1 在 0.4.0 重定口径：不再用被图像/对照稀释的全量 R@10 +10%，
改为 `bridge_zh` + `alias` 上的 nDCG 绝对增益。旧豁免整条删除。

T3 的豁免写明差值来自 Q7（宽泛上位词上扩展冲淡精排）；
**不接受为了转绿而把 Q7 相关集改回双零**。敏感探针上 P@5 是 +0.022。
