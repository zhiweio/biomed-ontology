...
linkml_meta = LinkMLMeta({'default_prefix': 'hmd',
     ...
     'description': '事实层（L4.5）与文档层（L4）模型。 仅做实体归一化只是"检索索引"，加上事实层与质量层才叫"数据底座"（设计决策 '
                    'D12）。\n'
     ...
     'imports': ['linkml:types', 'hmd_concept'],
     'license': 'Proprietary',
     'name': 'hmd_fact',
     ...
     'source_file': 'schema/hmd_fact.yaml',
...
class Synonym(ConfiguredBaseModel):
    ...
    confidence: Optional[float] = Field(default=None, ge=0.0, le=1.0, json_schema_extra = { "linkml_meta": {'domain_of': ['Synonym', 'Mapping', 'Fact']} })
...
class Mapping(ConfiguredBaseModel):
    ...
    confidence: Optional[float] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['Synonym', 'Mapping', 'Fact']} })
